/* Anv Daily Together — hybrid AI + local fallback */
(function(){
  const FOOD=[
  ...
  const AI_ENDPOINT=(typeof CONFIG!=='undefined'&&CONFIG.supabaseUrl)?`${CONFIG.supabaseUrl}/functions/v1/anv-daily-ai`:'';
  let aiBusy=false;
  ...
  function modal(){
    ...
    if(m)return m;
    m=document.createElement('div');m.id='anvDailyModal';m.className='anvDailyModal';m.innerHTML='<div class="anvDailyCard"><div class="anvDailyHead"><h3 id="anvDailyModalTitle"></h3><button class="anvDailyClose" type="button" aria-label="ปิด">×</button></div><div class="anvDailyResult" id="anvDailyResult"></div><div class="anvDailyHint" id="anvDailyHint">AI จะลองสร้างไอเดียใหม่ก่อน ถ้าใช้งานไม่ได้จะสุ่มจากชุดสำรองให้อัตโนมัติ ♡</div><div class="anvDailyActions"><button class="primary" type="button" id="anvDailyAgain">🎲 สุ่มใหม่</button></div></div></div>';
  ...
  async function askAI(type){
    ...
    aiBusy=true;
    try{
      const r=await fetch(AI_ENDPOINT,{method:'POST',headers:{'Content-Type':'application/json','apikey':key,'Authorization':`Bearer ${key}`},body:JSON.stringify({type})});
      if(!r.ok)return null;
  ...
  function open(type,title,list){
    const m=modal(),result=m.querySelector('#anvDailyResult'),hint=m.querySelector('#anvDailyHint');
    m.querySelector('#anvDailyModalTitle').textContent=title;m.classList.add('show');
    result.textContent='กำลังหาไอเดียใหม่ให้…';result.classList.add('anvDailyLoading');
    hint.textContent='AI จะลองสร้างไอเดียใหม่ก่อน ถ้าใช้งานไม่ได้จะสุ่มจากชุดสำรองให้อัตโนมัติ ♡';
    askAI(type).then(ai=>{
      result.classList.remove('anvDailyLoading');
      result.textContent=ai||pick(list);
      hint.textContent=ai?'✨ สุ่มใหม่จาก AI ได้อีก':'สำรอง: สุ่มจากรายการในเว็บ เพราะ AI ยังใช้งานไม่ได้';
    });
    m.querySelector('#anvDailyAgain').onclick=async()=>{
      result.textContent='กำลังสุ่มใหม่…';result.classList.add('anvDailyLoading');
      const ai=await askAI(type);result.classList.remove('anvDailyLoading');result.textContent=ai||pick(list);hint.textContent=ai?'✨ สุ่มใหม่จาก AI ได้อีก':'สำรอง: สุ่มจากรายการในเว็บ เพราะ AI ยังใช้งานไม่ได้';
    };
  }
  function init(){
    injectStyle();
    ...
    wrap.querySelector('[data-daily="food"]').addEventListener('click',()=>open('food','🍜 วันนี้กินอะไรดี?',FOOD));
    wrap.querySelector('[data-daily="activity"]').addEventListener('click',()=>open('activity','🍲 วันนี้ทำอะไรด้วยกันดี?',ACTIVITIES));
    wrap.querySelector('[data-daily="deep"]').addEventListener('click',()=>open('deep','💭 วันนี้คุยอะไรกันดี?',DEEP));
  }
